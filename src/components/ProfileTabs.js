    ...
    return (
        ...
        <div className={classes.root}>
            ...
            <TabPanel value={value} index={0}>
                <BasicProfile
                    values={props.basicData}
                    handleChange={(event) => props.handleChangeUser(event)}
                    postToUserApi={(event) => props.postToUserApi(event)}
                    handleChangeCountry={(event)=>props.handleChangeCountry(event)}
                    handleChangePicture={(data)=>props.handleChangePicture(data)}
                    open={props.open}
                    handleClose={(event,reason)=>props.handleClose(event,reason)}/>
            </TabPanel>
            <TabPanel value={value} index={1}>
                <GenralProfile
                    result={props.result}
                    method={(event) => props.method(event)}
                    postToApi={(event) => props.postToApi(event)}
                    open={props.open}
                    handleClose={(event,reason)=>props.handleClose(event,reason)}
                />
            </TabPanel>
